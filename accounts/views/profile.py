...
class UpdateProfileView(LoginRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy("accounts:profile", kwargs={"slug": self.request.user.username})

    def get_context_data(self, **kwargs):
        context_data = super().get_context_data(**kwargs)
        if "profile_form" not in context_data:
            context_data["profile_form"] = ProfileUpdateForm(instance=self.request.user.profile)
        return context_data

    def post(self, request, *args, **kwargs):
        data = request.POST
        user_form = self.form_class(data=data, instance=request.user)
        profile_form = ProfileUpdateForm(data=data, files=request.FILES, instance=request.user.profile)
        ...
        return self.render_to_response(context=self.get_context_data(form=user_form, profile_form=profile_form))
